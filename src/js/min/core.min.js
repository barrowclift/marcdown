/* Wrapping one-line pre */
for(var es=document.getElementsByTagName("pre"),i=0,e;e=es[i++];){var ls=e.textContent.split("\n");console.log(ls);for(var c=0,j=0,l=ls.length;l>j;j++)""!=ls[j]&&(c+=1);1==c&&(e.className+=" wrap")}
...
/* Settings */
$.bigfoot({actionOriginalFN: "ignore",numberResetSelector: "main"});
...
/* Settings */
$(document).ready(function() {$('h2,h3,h4,h5,h6').anchor({symbol: "¶"});});
/* Dark Mode */
$.cookie.defaults={path:'/'};if($.cookie("dark-mode")){var cssId='dark-mode';var head=document.getElementsByTagName('head')[0];var link=document.createElement('link');link.id=cssId;link.rel='stylesheet';link.type='text/css';link.href=darkModePath;link.media='all';head.appendChild(link);$("input[value='dark-mode']").prop("checked",true);}function darkModeToggled(checkbox){var cssId='dark-mode';if (checkbox.checked){$.cookie("dark-mode","on");if(document.getElementById(cssId)){$('link[rel=stylesheet][href$="'+darkModePath+'"]').prop("disabled",false);}else{var head=document.getElementsByTagName('head')[0];var link=document.createElement('link');link.id=cssId;link.rel='stylesheet';link.type='text/css';link.href=darkModePath;link.media='all';head.appendChild(link);}}else{$.removeCookie("dark-mode");if(document.getElementById(cssId)){$('link[rel=stylesheet][href$="'+darkModePath+'"]').prop("disabled",true);}}}